...
function _decodeJwtExp(token) {
  // Lee el claim `exp` (segundos epoch) directo del payload del JWT — no hace
  // falta la clave secreta para esto, el payload va en base64 sin firmar.
  // Antes _setToken calculaba su propio "hasta cuándo dura" (30 días) sin
  // mirar el token real, y el backend firma por 7 días (JWT_EXPIRE_HOURS) —
  // ese desfasaje hacía que el portal se mostrara "logueado" hasta 23 días
  // después de que el token ya había muerto del lado del servidor, y recién
  // se notaba cuando una acción puntual (ej. reclamar un lead) fallaba con
...
function _setToken(t) {
  if (!t) return;
  // Fuente de verdad: el exp real del JWT. Fallback de 7 días (igual al
  // default de JWT_EXPIRE_HOURS del backend) solo por si el token no trae exp.
  const expReal = _decodeJwtExp(t);
  const expiry = expReal || (Date.now() + 7 * 24 * 60 * 60 * 1000);
...
async function apiFetch(url, opts = {}) {
  ...
  // Sesión vencida: se maneja ACÁ una sola vez para los +75 lugares del portal
  // que llaman apiFetch directo (antes solo apiJSON —usado en 3 lugares— lo
  // hacía bien; el resto, como reclamarLead, mostraba tal cual el detalle
  // crudo del backend: "Token inválido o expirado."). Se excluye cualquier
  // URL de /login para no pisar el mensaje real de "contraseña incorrecta"
  // si quedó un token viejo dando vueltas en localStorage.
  if (res.status === 401 && tok && !/\/login\b/.test(url)) {
    _clearToken();
    if (typeof mostrarToast === 'function') mostrarToast('Tu sesión venció, ingresá de nuevo.', 'red');
...
async function apiJSON(url, method = 'GET', body = null) {
  ...
  if (r.status === 401) {
    // apiFetch ya limpió el token, avisó y programó el reload — acá solo
    // cortamos la ejecución del caller.
    throw new Error('401');
  }
  return r;
}
