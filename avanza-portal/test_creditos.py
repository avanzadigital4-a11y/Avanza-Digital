...
Cubrimos:
  ...
  8. Reclamo de lead (básico/calificado/premium): nunca descuenta créditos.
...
class TestCompraLeads:
    ...
    def test_compra_lead_premium_NO_descuenta_creditos(self, client, db, aliado, lead_premium, token_aliado):
        """Reclamar un lead premium ahora es GRATIS: no toca el saldo de créditos.

        Los créditos quedaron reservados exclusivamente para Jarvis IA.
        """
        aliado.creditos = 100
        db.commit()
        ...
        assert resp.status_code == 200
        db.refresh(aliado)
        assert aliado.creditos == 100  # sin cambio: los leads son gratis
    ...
    def test_compra_sin_creditos_igual_funciona(self, client, db, aliado, lead_premium, token_aliado):
        """Con saldo 0 igual se puede reclamar un premium: ahora es gratis."""
        aliado.creditos = 0
        db.commit()
        ...
        assert lead_premium.estado == "reclamado"
        db.refresh(aliado)
        assert aliado.creditos == 0  # no se cobró nada
